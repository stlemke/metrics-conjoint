...
if(isset($_POST['post-ranking'])) {
	...
	//Check that the input wasn't send via page refresh:
	$sql = "SELECT PubIDs_Order_Chosen FROM choices WHERE Respondent_ID = '".$respondent."'";
	$result = $conn->query($sql);
	$result_new = TRUE;
	foreach($result as $row) {
  ...
  <head>
	...
	<body>
		<div class="maincontainer" style="min-width:804px">
			...
			<form action="<?php if($_SESSION['finalTask']){echo 'p1p5.php';}else{echo 'nexttask.php';} ?>"  method="POST" name="startform" style="width:100%;font-size:13px" onsubmit="return validate(this);">
				<p>
				...
				<?php
					...
					$result = $conn->query($sql);
					//Load publication IDs of current choice set:
					foreach($result as $row) {
